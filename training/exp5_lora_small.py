...
OUTPUT_DIR    = os.path.expanduser("~/whisper_distil_nepali/models/exp5_whisper_small_lora")
SAMPLE_RATE   = 16000
BATCH_SIZE    = 16
LEARNING_RATE = 1e-4
NUM_EPOCHS    = 5
...
def main():
    ...
    accelerator.print("Exp 5 — LoRA Fine-tune Whisper Small (Cleaned Data)")
    ...
    accelerator.print(f"Trainable params: {sum(p.numel() for p in model.parameters() if p.requires_grad):,}")
